...
pipeline {
    ...
    stages {
        ...
        stage('Test Container') {
            steps {
                echo '🧪 Running test container with environment variables...'
                bat '''
                    echo Starting test container...
                    docker run -d --name test-flask -p 5000:5000 ^
                        -e MYSQL_HOST=db ^
                        -e MYSQL_USER=myuser ^
                    ...
                    echo Checking if Flask app is healthy...
                    curl -f http://localhost:5000 || (
                        echo ❌ Flask test failed — printing logs:
                    ...
                    echo ✅ Flask app responded successfully!
                    docker stop test-flask
                    docker rm test-flask
                '''
            }
        ...
        stage('Deploy') {
            ...
            steps {
                echo '🚀 Deploying new container on port 5000...'
                bat '''
                    echo Checking for any container on port 5000...
                    for /f "tokens=*" %%i in ('docker ps -a -q --filter "publish=5000"') do (
                        docker stop %%i
                        docker rm %%i
                    )
                    ...
                    echo Starting new container on port 5000...
                    docker run -d --name flask-app -p 5000:5000 %DOCKERHUB_REPO%:%IMAGE_TAG%
                '''
            }
        }
    ...
    post {
        ...
        failure {
            echo '❌ Build failed. Check above logs (especially Flask container logs).'
        }
    }
}
